...
from server_for_serveless.api.deps import AdminIdDep, PRESENCE_ONLINE_S, UserIdDep, presence_snapshot
...
from server_for_serveless.db.chat_sessions_db import (
...
from server_for_serveless.schemas.web import (
...
from server_for_serveless.services.chat_prefs import (
    DEFAULT_MAX_NEW,
    DEFAULT_TEMP,
    DEFAULT_TOP_P,
...
def _gp_prefs_kwargs() -> dict[str, Any]:
    rt = get_runtime()
    return {
        "inference_single_flight": bool(rt.inference_single_flight),
        "ui_block_cross_user_generation": bool(rt.ui_block_cross_user_generation),
        "ui_only": bool(rt.ui_only),
    }


def _apply_global_runtime_prefs_to_server(prev: dict[str, Any]) -> None:
    """Sincroniza o processo com app_global após o admin guardar. ``prev`` é o estado antes do UPDATE."""
    from server_for_serveless.inference.bootstrap import load_inference_backend

    p = get_global_runtime_prefs()
    rt = get_runtime()
    rt.apply_inference_queue_prefs(
        inference_single_flight=p["inference_single_flight"],
        ui_block_cross_user_generation=p["ui_block_cross_user_generation"],
    )
    prev_ui_only = bool(prev["ui_only"])
    if p["ui_only"]:
        rt.set_ui_only()
        return
    if prev_ui_only or not rt.is_loaded:
        try:
            load_inference_backend(rt)
        except Exception as err:
            set_global_runtime_prefs(
                inference_single_flight=prev["inference_single_flight"],
                ui_block_cross_user_generation=prev["ui_block_cross_user_generation"],
                ui_only=prev["ui_only"],
            )
            rt.clear()
            r2 = get_global_runtime_prefs()
            rt.apply_inference_queue_prefs(
                inference_single_flight=r2["inference_single_flight"],
                ui_block_cross_user_generation=r2["ui_block_cross_user_generation"],
            )
            if prev["ui_only"]:
                rt.set_ui_only()
            else:
...
def _user_settings_out(uid: int) -> UserModelSettingsOut:
    ...
    rt = get_runtime()
    if rt.backend in ("llama_server", "runpod") and rt.is_loaded:
        ls = get_llama_server_settings()
        return UserModelSettingsOut(
            is_admin=False,
            system_prompt=s["system_prompt"],
...
async def user_patch_settings(_uid: UserIdDep, body: UserModelSettingsIn):
    ...
    if admin:
        ...
        runtime_touch = (
            ...
            or body.ui_only is not None
        )
        if runtime_touch:
            prev = get_global_runtime_prefs()
            set_global_runtime_prefs(
                inference_single_flight=body.inference_single_flight,
                ui_block_cross_user_generation=body.ui_block_cross_user_generation,
                ui_only=body.ui_only,
            )
            _apply_global_runtime_prefs_to_server(prev)
    else:
...
async def index(request: Request):
    ...
    if not index_path.is_file():
        return HTMLResponse(
            "<p>Falta server_for_serveless/static/index.html</p>",
            status_code=500,
        )
...
async def page_login(request: Request):
    ...
    if not p.is_file():
        return HTMLResponse(
            "<p>Falta server_for_serveless/static/login.html</p>", status_code=500
        )
...
async def page_registar(request: Request):
    ...
    if not p.is_file():
        return HTMLResponse(
            "<p>Falta server_for_serveless/static/registar.html</p>", status_code=500
        )
    return FileResponse(p, media_type="text/html; charset=utf-8")
...
async def page_admin(request: Request):
    ...
    if not p.is_file():
        return HTMLResponse(
            "<p>Falta server_for_serveless/static/admin.html</p>",
            status_code=500,
        )
    return FileResponse(p, media_type="text/html; charset=utf-8")
...
async def api_admin_list_users(_admin: AdminIdDep) -> dict[str, list]:
    ...
    gen_uids = get_runtime().active_generation_user_ids
...
async def api_chat_generation_status(_uid: UserIdDep) -> dict[str, bool]:
    ...
    if inference_single_flight_enabled():
        ...
        if cross_user_ui_block_enabled():
            return {"active": True, "yours": yours}
        return {"active": yours, "yours": yours}
    uids = rt.active_generation_user_ids
    return {"active": uid in uids, "yours": uid in uids}
