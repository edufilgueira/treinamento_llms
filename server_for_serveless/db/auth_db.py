...
def get_global_runtime_prefs() -> dict[str, Any]:
    """Preferências globais (admin): fila de inferência, bloqueio de UI entre utilizadores, modo só interface."""
    with _db_lock:
        con = get_connection()
        try:
            with con.cursor() as cur:
                cur.execute(
                    """
                    SELECT inference_single_flight, ui_block_cross_user_generation, ui_only
    ...
    if not row:
        return {
            "inference_single_flight": True,
            "ui_block_cross_user_generation": True,
            "ui_only": False,
        }
    return {
        "inference_single_flight": int(row[0] or 0) == 1,
        "ui_block_cross_user_generation": int(row[1] or 0) == 1,
        "ui_only": int(row[2] or 0) == 1,
    }


def set_global_runtime_prefs(
    *,
    inference_single_flight: bool | None = None,
    ui_block_cross_user_generation: bool | None = None,
    ui_only: bool | None = None,
) -> dict[str, Any]:
    cur_prefs = get_global_runtime_prefs()
    sf = (
        bool(inference_single_flight)
        if inference_single_flight is not None
        else cur_prefs["inference_single_flight"]
    )
    ub = (
        bool(ui_block_cross_user_generation)
        if ui_block_cross_user_generation is not None
        else cur_prefs["ui_block_cross_user_generation"]
    )
    uo = bool(ui_only) if ui_only is not None else cur_prefs["ui_only"]
    with _db_lock:
        con = get_connection()
        try:
            with con:
                with con.cursor() as cur:
                    cur.execute(
                        ...
                        UPDATE app_global SET
                            inference_single_flight = %s,
                            ui_block_cross_user_generation = %s,
                            ui_only = %s
                        WHERE id = 1
                        """,
                        (1 if sf else 0, 1 if ub else 0, 1 if uo else 0),
                    )
        finally:
            con.close()
    return get_global_runtime_prefs()
...
def set_llama_server_settings(
    *,
    ...
    from server_for_serveless.services.llama_context import max_allowed_max_new_tokens_for_n_ctx
