...
Configuração por variáveis de ambiente (`.env` na raiz do projecto ou `server_for_serveless/.env`;
o segundo substitui o primeiro) ou `export` no shell. **ORACULO_PG_HOST é obrigatório**
(o servidor não arranca sem ele — ver `server_for_serveless/main.py` / `serve_lora.py`).
...
def _load_dotenv_if_available() -> None:
    """Carrega `.env` na raiz do projecto e opcionalmente `server_for_serveless/.env` (este sobrepõe)."""
    try:
...
def get_pg_dsn_dict() -> dict[str, Any]:
    ...
    if not host:
        raise RuntimeError(
            "ORACULO_PG_HOST não definido. Define-o no `.env` (copia server_for_serveless/.env.example → server_for_serveless/.env)."
        )
    _pg_params = {
        "host": host,
...
def _ensure_app_global_runtime_columns(cur: Any) -> None:
    """Migração idempotente: fila global / bloqueio UI / modo só interface (admin, app_global)."""
    cur.execute(
        """
        SELECT column_name FROM information_schema.columns
        WHERE table_schema = 'public' AND table_name = 'app_global'
